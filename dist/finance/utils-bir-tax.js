...
exports.computeGraduatedIncomeTax = computeGraduatedIncomeTax;
exports.getFilingDatePercentageTax = getFilingDatePercentageTax;
exports.getFilingDateIncomeTax = getFilingDateIncomeTax;
exports.getFilingDateWithholdingTaxMonthly = getFilingDateWithholdingTaxMonthly;
exports.getFilingDateWithholdingTaxAnnual = getFilingDateWithholdingTaxAnnual;
exports.getFilingDateVAT = getFilingDateVAT;
 ...
 */
function computeGraduatedIncomeTax(income, type) {
    const FLAT_TAX_THRESHOLD = 250000;
    const FLAT_TAX_RATE = 0.08;
    if (type === enums_finance_1.IncomeTaxType.FLAT) {
    ...
    if (income <= 800000)
        return 22500 + (income - 400000) * 0.2;
    if (income <= 2000000)
        return 102500 + (income - 800000) * 0.25;
    if (income <= 8000000)
        return 402500 + (income - 2000000) * 0.3;
 ...
 */
function getFilingDatePercentageTax(quarter, year = getCurrentYear()) {
    const quarterEndDates = {
        [enums_finance_1.QuarterPeriod.Q1]: new Date(year, 2, 31), // March 31
        [enums_finance_1.QuarterPeriod.Q2]: new Date(year, 5, 30), // June 30
        [enums_finance_1.QuarterPeriod.Q3]: new Date(year, 8, 30), // September 30
        [enums_finance_1.QuarterPeriod.Q4]: new Date(year, 11, 31), // December 31
    };
 ...
 */
function getFilingDateIncomeTax(quarter, year = getCurrentYear()) {
    if (quarter === enums_finance_1.QuarterPeriod.Q4) {
    ...
    const quarterFilingDates = {
        [enums_finance_1.QuarterPeriod.Q1]: new Date(year, 4, 15), // May 15
        [enums_finance_1.QuarterPeriod.Q2]: new Date(year, 7, 15), // August 15
        [enums_finance_1.QuarterPeriod.Q3]: new Date(year, 10, 15), // November 15
    };
 ...
 */
function getFilingDateWithholdingTaxMonthly(month, year = getCurrentYear()) {
 ...
 */
function getFilingDateWithholdingTaxAnnual(year = getCurrentYear()) {
    return new Date(year + 1, 0, 31).toISOString().split('T')[0]; // January 31
}
/**
 * VAT Filing Date - 25 days after the end of each quarter
 */
function getFilingDateVAT(quarter, year = getCurrentYear()) {
    const quarterEndDates = {
        [enums_finance_1.QuarterPeriod.Q1]: new Date(year, 2, 31), // March 31
        [enums_finance_1.QuarterPeriod.Q2]: new Date(year, 5, 30), // June 30
        [enums_finance_1.QuarterPeriod.Q3]: new Date(year, 8, 30), // September 30
